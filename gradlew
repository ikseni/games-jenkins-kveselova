...
if [ "$cygwin" = "true" -o "$msys" = "true" ] ; then
    ...
    for i do
        arg="$i"
        if [ $CC_UNCONVERTED = true ] && [[ "$arg" =~ $OURCYGPATTERN ]] ; then
            i=`cygpath --path --ignore --mixed "$arg"`
        else
            i="$arg"
        fi
        # Roll the args list around exactly as many times as the number of
        # args, so each arg winds up back in the $1 position
        for shift do
            set -- "$@" "$1"
            shift
        done
...
if [ "$cycle" = "true" ] ; then
    eval splitJvmOpts `$JVM_OPTS_SPLIT`
    JAVA_OPTS="$splitJvmOpts"
    GRADLE_OPTS="$splitJvmOpts"
else
    JAVA_OPTS="$JVM_OPTS"
